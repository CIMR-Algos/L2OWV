...
$\hat{T}_{bd}(z_k)$ is a brightness temperature *rate* with
...
are not identical. Since we must model scenes for a satellite radiometer within the antenna patterns, we also need the self-attenuated atmospheric 
emission at the flight level in an arbitrary direction whithin the antenna patterns.  In general this is
...
even at Ku, Ka, X, C and L-bands. Here we choose to simplify the problem by neglecting
...
owing to self-attenuated atmospheric emission reduces to one possible integral when the path is directed upward
the earth's surface:
...
where $\hat{T}_{ba}(z')$ is given by [Equation](eqatm72) evaluated at
...
into [Equation](eqatm79), we obtain the approximate brightness temperature
...
integrals, we may precompute these integrals for a fixed antenna
elevation and store the results for later calculations at arbitrary
...
model for self-attenuated Ku to L-band atmospheric brightness temperature
...
The polarization vector of an electromagnetic wave of frequency $f$ [Hertz]
in the microwave range that propagates from the Earth to the S/C through the geomagnetic field and the Earth’s
ionosphere undergoes a rotation (Faraday rotation) by the angle $\Omega$ ({cite:t}`yueh2000estimates`;{cite:t}`Meissner2006`):
...
field vector [Gauss] and $d\vec{s}$ is the vector line element in the direction of propagation.
When looking into the propagation
direction of the electromagnetic wave, the electric field
polarization vector rotates clockwise if $\Omega>0$, i.e., if the geomagnetic
field is pointing along the direction of propagation.
...
According to [Equation](eqFara1),  the magnitude of the Faraday rotation angle
grows with $1/f^2$. For CIMR-like Earth incidence angles and
all typical ocean scenes $Q_{hv}>>U_{hv}$. Therefore, according to [Equation](eq:rot1),
for typical values of $\Omega$, the relative impact of Faraday rotation on the third Stokes parameter is much larger than on the second
Stokes parameter (vertical and horizontal polarizations). For
the CIMR instrument, Faraday rotation plays a significant role
only for the third Stokes parameter at 1.4, 6.9 and 10.7 GHz.  At 1.4 GHz, the
size of the Faraday rotation is more than 50 times larger than at 10.7 GHz.
If not corrected, this would lead to a large relative error
in the V- and H-pol brightness temperatures and therefore to inaccurate
values for the OWV.

In order to compute the Faraday rotation for CIMR orbits
we assume a spherical earth (radius km) and
use the thin layer approximation ({cite:t}`LeVine2000`), which assumes that all
electrons are concentrated at the ionospheric layer at an altitude $h_I$=400 km above mean sea level:
...
is the Vertical Total Electron Content (VTEC) of the ionosphere
between the Earth’s surface and the S/C altitude $h_{S/C}$. We can
then substitute the full profile integral [Equation](eqFara1) by
 ...
 *$h_{S/C}$* using the formulation of {cite:t}`Floury2007`, $B_I$ is the magnetic field strength [Tesla] evaluated at the ionospheric pierce point (IPP), the point where the ray from the spacecraft to the 
...
The $\mathrm{VTEC}(z=\infty,lat_{400},lon_{400})$ can be obtained from the 1-day forecast produced Centre for Orbit Determination in Europe (CODE), University of Berne,
 Switzerland. For future reprocessed OVW products, the VTEC can be obtained from IGS consolidated VTEC.
...
- $T_{atm}^{up}$ and $T_{atm}^{dw}$ are the unpolarized upwelling and downwelling brightness temperature of atmospheric 1-way emission which can be derived from [Equation 4](#equation-eq2) and  [Equation 132](#equation-eqatm84),
- $τ_d$ is the 1-way atmospheric transmittance associated with molecular oxygen absorption, determined from [Equation 119](#equation-eqatm71),
- $τ_v$ is the 1-way atmospheric transmittance associated with water vapor absorption, , determined from [Equation 119](#equation-eqatm71),
- $T_{surf,p}^{tot}$ and $R_{surf,p}^{tot}$, the p-pol brightness temperature, and reflectivity, of the total sea surface emission, including specular emission determined from [Equation 38](#equation-eq19),[Equation 39](#equation-eq20); rough and foamy sea surface
induced emission from [Equation 43](#equation-eq-rough4) to [Equation 46](#equation-eq-rough7),
- $T_{erU}$	and $T_{erV}$ are the third and fourth Stokes brightness temperature of rough surface emission, respectively, determined in from [Equation 46](#equation-eq-rough7) 
...
where $\theta_s$ is the incidence angle and $r$ is the range. For a spherical Earth, the leading term $f_{lat}$ would
...
which  is the unit vector pointing from the antenna to $dA$. The term $\mathbf{\Psi}(\phi)$ is the rotation matrix defined in previous section ([Equation (165)](#equation-eq-rot10)).
...
The gain functions $G_i(x,y)$ of the individual CIMR observations are given by the pre-launch measured antenna patterns of the CIMR multi-frequency chanels and
 are the individual CIMR gain patterns of the effective field of view (EFOV) of 36 x 64 km for L-band, 11x19 km for C-band, 7x13 km for X-band, 4x6 km for Ku-band and 3x5 km 
 for Ka-band.
...
The input data from CIMR are the "eight-flavors" CIMR Leve1b chanel resampled four stokes parameters for the aft- and fore view of the instrument at the Top of the Atmosphere for the 5 frequencies,
 namely, $T_{th}^{TOA}$, $T_{tv}^{TOA}$, $U^{TOA}$, and $V^{TOA,aft}$.  Other input data from CIMR are the L2 sea surface temperature and sea surface salinity, also properly resampled on the CIMR 
 Leve1b X-band channel grid.  In case, these two variables are not available, the algorithm will also rely on ECMWF weather forecasts and CMEMS model.
  It will also provide an ensemble of auxilliary data (vertical profiles of temperature, water vapor, etc..) needed for evaluating the atmospheric corrections.   
  
In the proposed processing, we allow the wind speed vector to vary within a region about the a priori wind speed, direction and surface temperature and salinity 
(which shall be derived from the multi-frequency chanels of CIMR, or, from a numerical weather or ocean forecasts such as ECMWF and CMEMS) via additional penalty terms in the objective function 
while leaving  the wind vector unconstrained. 
To retrieve OWV, we will use a Maximum-likelihood method with the following objective sum of square function:
...
The environmental parameters used to calculate RTM $T_B$ include the sea surface temperature $T_s$ and salinity $S$, the surface wind speed $U_{10}$ and wind direction $\phi_{w}$, the 
columnar water vapor $V$, and columnar cloud liquid water $L$. The Earth incidence angle 𝜃s and the azimuthal look angle $\phi_s$ depend on the channel *i* at frequency $f$ and on whether the observation is from forward or backward look. 
The sum in [Equation](eq:MLE) runs over all channels of the five polarimetric frequency bands: 1.4, 6.9, 10.7, 18.7, and 36.5 GHz. The inverse weights of these channels 
are the expected variances, i.e., squares of the expected errors. The weighting factors for the CIMR data are set according to the expected measurement
and modeling uncertainties. We let $NEDT_i$ be the Noise-Equivalent-Delta-T (NEDT) of the radiometer chanel *i* at frequency *f*. 

The variables $U_{10}^{p}$, $\phi_{w}^{p}$, $S^{p}$ and $T_{s}^{p}$ are the *a priori* values for the auxilliray surface wind speed, wind direction (with uncertainties $\delta_{U_{10}}$, $\delta_{\phi_{w}}$), 
salinity and temperature, respectively. 

We apply the conjugate gradient technique using a modified Levenberg-Marquardt algorithm {cite:t}`Marquardt1963` to find the local minima.
The wind speed vector solution to this constrained minimization problem is the final L2B retrievals.

For the two-look retrieval, the MLE typically finds 2–
3 minima, called ambiguities. The ambiguities are ranked by the
value of chi-square in ascending order. Thus, the first-ranked
ambiguity is the solution with the smallest chi-square residual.
If there were no errors in the measured $T_B$ or in the RTM $T_B$,
then the first-ranked ambiguity would always be the correct
solution. However, the presence of errors can cause incorrect
solutions to have the lowest chi-square value. This is most
likely to occur when the dependence of $T_B$ on the relative wind
direction is small, which happens at low wind speeds

When an OVW solution is ambiguous, spatial information
can be used to break the ambiguity. Median fil a
long history in OVW retrieval {cite:t}`hilburn2015ocean`. It is important to understand that the wind
vectors themselves are not being median filtered. Insted, rather
than choosing the first-ranked ambiguity, the ambiguity closest
to the area-median direction is chosen. The median filter must
be initialized with some wind field. One choice is to use the
first-ranked ambiguity; however, the best results are obtained
using a smoother wind field from numerical weather prediction.
This does raise the potential of selecting the wrong solution if
the numerical weather prediction winds do not have the same
meteorological features in the same locations as observed by
satellite. For the purpose of this ATBD, the selected ambiguity
refers to the ambiguity resulting from the median filtering process initialized with winds from
ECMWF. The benefit of a two-look configuration is increased resilience to errors in measured $T_B$ and in the RTM $T_B$.
